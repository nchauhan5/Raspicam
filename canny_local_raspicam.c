...
void canny(unsigned char *image, int rows, int cols, float sigma, float tlow,
		float thigh, unsigned char **edge, char *fname);
...
void gaussian_smooth(unsigned char *image, int rows, int cols, float sigma,
	...
	*kernel, /* A one dimensional gaussian kernel. */
	...
	for (j = 0; j < N_T; j++) {
		argsY[j].rows = rows;
		argsY[j].cols = cols;
		argsY[j].row_s = row_per_t * j;
		argsY[j].row_e = row_per_t * (j + 1);
		argsY[j].center = center;
		argsY[j].kernel = kernel;
		argsY[j].tempim = tempim;
		argsY[j].smoothedim = smoothedim;
		iret1[j] = pthread_create(&threadY[j], NULL, &blur_y, &argsY[j]);
	}

	for (j = 0; j < N_T; j++) {
		pthread_join(threadY[j], NULL);
	}
...
void non_max_supp(short *mag, short *gradx, short *grady, int nrows, int ncols,
	...
	for (rowcount = 1, magrowptr = mag + ncols + 1, gxrowptr = gradx + ncols
			+ 1, gyrowptr = grady + ncols + 1, resultrowptr = result + ncols + 1;
